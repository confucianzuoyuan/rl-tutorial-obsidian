...
for batch_idx in range(total_batches):
    ...
    # 获取当前批次的偏好对数据
    preferred_batch_sequences = chosen_input_ids_list[batch_idx * batch_size:(batch_idx + 1) * batch_size]
    rejected_batch_sequences = rejected_input_ids_list[batch_idx * batch_size:(batch_idx + 1) * batch_size]
    ...
    # 参考模型的前向传播（不计算梯度）
    with torch.no_grad():
        reference_preferred_logits = model_ref(preferred_model_inputs)
            .logits
            .detach()
        reference_rejected_logits = model_ref(rejected_model_inputs)
            .logits
            .detach()
    ...
    sample_losses = (-1) * torch.log(preference_probability)
    ...
    if should_log:
        ...
        # 调用外部日志记录
        log_call(
            batch_idx, recent_loss, recent_preferred_logprob, 
            recent_rejected_logprob, recent_preferred_reward, 
            recent_rejected_reward, recent_margin
        )
