...
\end{aligned}
$$
我们的目标是让 $J(\theta)$ 越大越好，那么在经过梯度上升算法之后，$J(\theta)$会变大，那么肯定有的 $\log\pi_\theta(A_t|S_t)$ 变大了，也可能有的 $\log\pi_\theta(A_{t'}|S_{t'})$ 变小了，但总和肯定变大了。也就是说，
\bigskip
策略梯度法的本质是，我们使用\textbf{当前策略}$\pi_\theta$采样一条轨迹$\tau$，如果采样的轨迹的回报$G(\tau)$很高，那么说明我们在某个状态$S_t$采取的动作$A_t$比较好，导致了总的奖励也就是回报比较高。那么通过梯度上升法，更新$\theta$之后，新的策略$\pi_{\theta_\text{new}}$会提升\textbf{在 $S_t$ 时执行 $A_t$ 的概率}。例如 $A_t$ 是``向左推车''，那么在 $S_t$ 状态下策略采取``向左推车''的概率就会提升，而同时策略采取``向右推车''的概率就会下降。
\bigskip
...
\mybox[blue]{蒙特卡洛采样}{
  我们之前使用的是蒙特卡洛采样，也就是采样出一条完整的轨迹。然后更新策略神经网络$\pi_\theta$。
}
...
例如：当我们使用旧策略玩了一把倒立摆游戏之后，保存的轨迹中$\pi_{\theta_\text{old}}(a_1|s_1)=\left[ \text{向左推的概率}=0.3, \text{向右推的概率}=0.7 \right]$。那么新策略也可以针对$s_1$计算一下采取动作的概率分布。
...
我们有6种不同的情况。首先记住，我们取裁剪目标和非裁剪目标中的最小值。
...
本节我们引入相对策略性能恒等式，用于度量两个策略之间的性能差异。随后展示如何利用该恒等式改造策略梯度目标，从而在优化过程中保证性能的单调提升。

直观上，问题出在步长上，因此可以通过加入一个约束，将步长限制在安全范围以防止性能崩塌。施加恰当的约束后，我们可以推导出所谓的单调改进理论。

首先，给定一个策略$\pi$，其参数更新后的下一次迭代为$\pi'$。我们将相对策略性能恒等式定义为二者目标函数的差值，如式（7.5）所示。需要注意，优势函数 $A^{\pi}(s_t,a_t)$ 始终使用旧策略 $\pi$ 来计算。
...
\begin{aligned}
J(\pi')-J(\pi) &= \mathbb{E}_{\tau\sim\pi'}\left[ \sum_{t\geq 0}A^\pi(s_t,a_t) \right] \\
& \approx \mathbb{E}_{\tau\sim\pi}\left[ \sum_{t\geq 0}A^\pi(s_t,a_t)\frac{\pi'(a_t|s_t)}{\pi(a_t|s_t)} \right] \\
&= J^{\text{CPI}}_\pi(\pi')
\end{aligned}
\end{equation}
...
常数 $C$ 需要被选择，KL 表示 KL 散度。式（7.25）表明，如果相邻策略 $\pi,\pi'$ 的分布彼此接近，使得右侧的 KL 散度较低，那么左侧的误差项也会较小。误差较小时，$J_\pi^\text{CPI}(\pi')$ 就是对 $J(\pi')-J(\pi)$ 的良好近似。

基于此，要推导我们想要的结果 $J(\pi')-J(\pi)\geq 0$ 相当直接。这被称为单调改进理论，因为它保证每次策略迭代至少是正的或零的改进，而不会出现退化。为得到该结论，首先展开式（7.25），并考虑其在式（7.26）中的下界。

\begin{equation}
J(\pi')-J(\pi)\geq J_\pi^\text{CPI}(\pi') - C\sqrt{\mathbb{E}_t[KL\left(\pi'(a_t|s_t)||\pi(a_t|s_t)\right)]}
\end{equation}

现在，让我们看看策略迭代某一步中的最坏情况。考虑对新策略 $\pi'$ 的所有选择，其中也包含旧策略 $\pi$（参数更新幅度为 0）。如果没有候选策略表现更好，就直接设定 $\pi'=\pi$，并在该次迭代中不进行更新。在这种情况下，式（7.17）告诉我们$J_\pi^\text{CPI}(\pi)=\mathbb{E}_{\tau\sim\pi}\left[\sum_{t\geq 0}A^\pi(a_t|s_t)\frac{\pi(a_t|s_t)}{\pi(a_t|s_t)}\right]=0$，因为一个策略相对于自身没有期望优势。根据恒等式，KL 散度满足 $KL(\pi||\pi)=0$。

为了接受策略的变更，式（7.26）表明，估计的策略改进 $J_\pi^\text{CPI}(\pi')$ 必须大于最大误差 $C\sqrt{\mathbb{E}_t[KL\left(\pi'(a_t|s_t)||\pi(a_t|s_t)\right)]}$。
...
这一结果满足了我们的最终要求。它使我们能够避免在使用原始目标 $J(\pi)$ 时可能出现的性能崩溃。需要注意的一个关键区别是，单调改进并不保证收敛到最优策略 $\pi^*$。例如，策略优化仍可能停留在一个局部极大值处，在该处每次策略迭代都不产生改进——即$J(\pi')-J(\pi)=0$。保证收敛仍是一个困难的开放问题。

最后一步是考虑如何在实践中实现式（7.27）提出的优化问题。一个思路是直接约束 KL 的期望，如式（7.28）所示。

\begin{equation}
\mathbb{E}_t[KL\left(\pi'(a_t|s_t)||\pi(a_t|s_t)\right)]\leq\delta
\end{equation}

$\delta$限制了 KL 散度的大小，因此实际上约束了新策略 $\pi'$ 相对旧策略 $\pi$ 能偏离的程度。只有策略空间中位于 $\pi$ 附近的小邻域内的候选策略才会被考虑。这个邻域称为置信域（trust region），而式（7.28）称为置信域约束。需要注意的是，$\delta$是一个需要调参的超参数。

约束项 $\mathbb{E}_t[KL\left(\pi'(a_t|s_t)||\pi(a_t|s_t)\right)]$ 被写成对单个时间步 $t$ 的期望，因此将目标 $J_\pi^\text{CPI}(\pi')$ 也以这种形式书写是有帮助的，如式（7.29）所示。除此之外，由于我们将对 $\theta$ 最大化该目标，让我们也按照式（7.30）将策略用 $\theta$ 表达。新策略写作 $\pi'=\pi_\theta$，而带有固定参数的旧策略写作 $\pi=\pi_{\theta_\text{old}}$。由旧策略计算的优势也可简记为 $A^\pi(s_t,a_t)=A^{\pi_{\theta_\text{old}}}_t$。
...
将约束与代理目标合并后，置信域策略优化问题如式（7.31）所示。
