...
PPO 的第一个变体称为带自适应 KL 惩罚的 PPO，其目标如式（7.33）所示。它将 KL 约束 $\mathbb{E}_t[KL\left(\pi'(a_t|s_t)||\pi(a_t|s_t)\right)]\leq\delta$ 转换为一个自适应的 KL 惩罚，并从重要性加权的优势中减去该惩罚。其结果的期望就是需要最大化的新目标。

\begin{equation}
J^{\text{KLPEN}}(\theta) = \max_\theta\left[ p_t(\theta)A_t - \beta KL\left(\pi'(a_t|s_t)||\pi(a_t|s_t)\right) \right]
\end{equation}

式（7.33）被称为带 KL 惩罚的代理目标。$\beta$ 是一个自适应系数，用于控制 KL 惩罚的大小。它在控制优化的置信域方面与式（7.31）起到相同的作用。$\beta$ 越大，$\pi_\theta$ 与 $\pi_{\theta_\text{old}}$ 的差异越小。$\beta$ 越小，两种策略之间的容忍度越高。

使用常数系数的一个挑战在于，不同问题具有不同特性，因此很难找到一个在所有场景都适用的数值。即使在单一问题上，随着策略迭代，损失景观也会变化，因此先前有效的 $\beta$ 可能在后续不再有效——它需要随这些变化进行自适应。

为解决该问题，PPO 论文提出了一个基于启发式的 $\beta$ 更新规则，使其能够随时间自适应。$\beta$ 在每次策略更新后都会被更新，其新值在下一次迭代中使用。$\beta$ 的更新规则见算法 7.1。它可以作为本节稍后将介绍的 PPO 算法中的一个子程序来使用。
...
\State 使用多轮小批次SGD来优化（最大化）KL惩罚替代目标$J^{\text{KLPEN}}(\theta) = \mathbb{E}_t\left[ p_t(\theta)A_t - \beta KL\left(\pi_\theta(a_t|s_t)||\pi_{\theta_\text{old}}(a_t|s_t)\right) \right]$
...
基于裁剪代理目标的 PPO 通过去除 KL 约束，并对式（7.30）中的代理目标作更简单的修改来弥补这一点，如式（7.34）所示。

\begin{equation}
J^{\text{CLIP}}(\theta) = \mathbb{E}_t\left[ \min\left( p_t(\theta)A_t, \text{clip}\left( p_t(\theta), 1-\epsilon, 1+\epsilon \right)A_t \right) \right]
\end{equation}

式（7.34）被称为裁剪代理目标。$\epsilon$是定义裁剪邻域$|p_t(\theta)-1|\leq\epsilon$的数值。它是一个需要调参的超参数，且可在训练过程中衰减。$\min(\cdot)$内的第一项就是代理目标$J^\text{CPI}$。第二项 $\text{clip}\left( p_t(\theta)A_t, 1-\epsilon, 1+\epsilon \right)$ 将 $J^\text{CPI}$ 的取值限制在 $(1-\epsilon)A_t$ 与 $(1+\epsilon)A_t$ 之间。当 $p_t(\theta)$ 位于区间 $[1-\epsilon,1+\epsilon]$ 内时，$\min(\cdot)$ 中的两项相等。
