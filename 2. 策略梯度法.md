...
这里有一个问题，那就是真正的期望是无法准确求出来的。因为期望是所有的轨迹得到的奖励计算出来的。而轨迹有无数条。如果我们能够走满倒立摆的 200 步，那么不同轨迹的数量可能是 $2^{200}$ 。这是一个天文数字。所以我们希望能够求出 **近似** 期望值的数值。比如利用大数定理，多采样几条轨迹，那么就会比较接近期望值。
...
class Agent:
	...
	def add(self, reward, prob):
	    """保存每一个时间步的奖励和动作的概率，供后面计算"""
		data = (reward, prob)
		self.memory.append(data)

	def update(self):
		G, loss = 0, 0
		# 逆序遍历保存的奖励，计算回报（秦九韶算法）
...
![[reinforce-pg-code.excalidraw|1000]]

我们使用可视化代码将回合和奖励绘制出来。

从图中可以看出，随着回合的推进，奖励的总和会逐渐增加。与上一次的结果相比，不但训练稳定了，训练速度也提高了。

![[REINFORCE算法收益变化图.png]]
...
**当前状态的价值 = 采取一步行动后获得的即时奖励 + 折扣因子 * 下一个状态的价值**

我们评估一家所在公司的前景（当前公司对我们的价值，也就是当前状态的价值），不应该只考虑这个月给我们发的工资（采取一步行动后获得的即时奖励），还应该考虑之后公司对我们的价值（下一步状态的价值）打个折扣。毕竟当前的钱比较重要，未来的前景也重要，但没那么重要。
```
...
Actor-Critic 中的 Actor 是 **演员（采取动作的人）** 的意思，也就是采取动作的人，相当于策略 $\pi_{\theta}$ 。而 Critic 是 **评论家** 的意思，相当于价值函数 $V_{\omega}$ 。因此，Actor-Critic 的意思是“使用 $V_{\omega}$ ，来评论基于策略 $\pi_{\theta}$ 采取的动作的好坏”。
```

```ad-attention
title: 1步TD误差

注意：我们这里使用 ==1== 步TD误差来估计优势。

优势就是：使用价值函数神经网络 $V_{\omega}$ ，来评论基于策略 $\pi_{\theta}$ 采取的动作的好坏

我们也可以使用 $n$ 步TD误差来估计优势。叫做 [[附录C：广义优势估计]]
```
...
class Agent:
	...
	def update(self, state, action_prob, reward, next_state, done):
		...
		target = target.detach() # 从计算图中剥离出来，required_grad = False
		# V_ω(S_t)
		v = self.v(state)
		loss_fn = nn.MSELoss()
...
![[actor-critic-code-comments.excalidraw|1000]]
...
上面4种方法的权重 $\Phi_t$ 各不相同。最简单的策略梯度法在所有时刻的权重都是 $G_\tau$ 。对它进行改进，将时刻 $t$ 的收益 $G_t$ 作为权重进行评估的是REINFORCE。另外，通过加入“基线”这一方法，减小了方差的方法是带基线的REINFORCE。Actor-Critic是一种除了策略之外，对价值函数也用神经网络建模的方法。期待 (1)⟶(2)⟶(3)⟶(4) 这样的编号一直延续下去，从而有更高级的方法出现，产生更好的结果。
