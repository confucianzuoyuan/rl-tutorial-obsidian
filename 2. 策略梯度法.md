...
$\text{Actor-Critic}$ 中的 $\text{Actor}$ 是 **演员（采取动作的人）** 的意思，也就是采取动作的人，相当于策略 $\pi_{\theta}$ 。而 $\text{Critic}$ 是 **评论家** 的意思，相当于价值函数 $V_{\omega}$ 。因此，$\text{Actor-Critic}$ 的意思是“使用 $V_{\omega}$ ，来评论基于策略 $\pi_{\theta}$ 采取的动作的好坏”。
```
