...
我们可以将策略神经网络和价值函数模型融合在一起，这样反向传播可以一起训练。
...
```ad-note
上面尤其要注意的一点是，我们的价值函数就是在原始 `llm_model` 的基础上，添加一个 **Value Head** 线性层作为价值函数（critic），这个 critic 的输入是生成的文本，输出是一个标量值。这个标量值表示的是当前文本的价值怎么样。

所以，会发现我们的价值函数并没有单独训练一个模型，而是在 actor 的后面加了一层线性层，作为 critic ，这样可以在反向传播的时候一起更新 actor 和 critic 。
```
...
接下来我们使用 [[附录C：广义优势估计（GAE）]] 来计算 `ppo_model` 输出的优势。我们的 $\lambda=1$ 。
