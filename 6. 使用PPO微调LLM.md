...
print(len(train_dataloader)) # 1544
...
我们的思路还是将一个 124M 参数的 gpt2 预训练模型微调成为一个奖励模型。
...
在倒立摆游戏中，$\text{actor}$ 模型是玩游戏的推车。

在 PPO-RLHF 中，$\text{actor}$ 模型是我们在sft过的大语言模型gpt2。

在RLHF中，$\text{critic}$ 是一个线性层。用来评判gpt2-sft的输出的价值是多少。
