...
我们使用 sst2 数据集。这是一份电影评论的数据集。质量很高。

![[Pasted image 20250908134958.png]]

标签是评论的情感。我们现在只使用 **文本部分** 来对 gpt2 进行监督微调。
...
之前在使用 PPO 玩倒立摆游戏时，我们是不需要奖励模型的，因为没有木杆只要不倒下，倒立摆环境就会给出即时奖励 1 。
...
我们的思路还是将一个 124M 参数的 gpt2 预训练模型微调成为一个 **奖励模型** 。
...
我们在使用 PPO 玩倒立摆时，分别训练了 $\text{actor}$ 模型和 $\text{critic}$ 模型，共两个模型。

在倒立摆游戏中，$\text{actor}$ 模型是玩游戏的推车的策略模型。

在 PPO-RLHF 中，$\text{actor}$ 模型是我们在sft过的大语言模型gpt2-sft。

在 PPO-RLHF 中，$\text{critic}$ 是一个线性层。用来评判 gpt2-sft 的输出的价值是多少。
...
而由于我们的大语言模型gpt2-sft也是需要更新的（更新的目的是让llm输出我们偏好的回答，也就是正向情感的影评）。所以我们可以把actor和critic合并到一个模型中，一起更新权重。

$\text{critic}$ 线性层在 PPO-RLHF 中叫做“价值头”（value head）。定义如下
...
![[Pasted image 20250908161416.png]]
...
$$
\begin{aligned}
\text{reward} &= \text{score} - \log\left(\frac{\pi_\theta^{\text{RL}}}{\pi^{\text{SFT}}}\right) \\ &= \text{score} - (\log\pi_\theta^{\text{RL}}-\log\pi^{\text{SFT}})
\end{aligned}
$$
...
- actor: gpt2-sft
- critic: value head
- reward model: 奖励模型（这个还需要单独训练，很难训练！）
- 冻结的参考模型gpt2-sft
```
