...
PPO 算法是 Actor-Critic 架构的，也就是 **演员-评论家** 架构。
...
其中 $A_t^{\pi_\theta} = R_t+\gamma V_\omega(S_{t+1})-V_\omega(S_t)$ 。这里的 $A$ 表示优势，也就是 Advantage 。表示当前策略采取动作 $A_t$ 之后，相对于价值函数评估的价值的优势是多少。
...
如果新策略与旧策略的概率比超出 $(1-\epsilon)$ 和 $(1+\epsilon)$ 的范围，则优势函数将被剪裁。在 $\text{PPO}$ 论文中的实验中，$\epsilon$ 设置为 $0.2$ 。
